...
import { EsgDashboardService } from "../services/EsgDashboardService";

import AverageScoreByTier from "./AverageScoreByTier";

import DashboardKpiCards from "./DashboardKpiCards";

import { IEsgDashboardProps } from "./IEsgDashboardProps";

import OverallEsgScore from "./OverallEsgScore";

import PendingReviewAging from "./PendingReviewAging";

import QualificationBreakdown from "./QualificationBreakdown";

import QuickInsights from "./QuickInsights";

import RecentSubmissionsSnapshot from "./RecentSubmissionsSnapshot";

import RiskRatingAnalysis from "./RiskRatingAnalysis";

import SubmissionsByTier from "./SubmissionsByTier";

import SubmissionsTrend from "./SubmissionsTrend";

import TopPendingActions from "./TopPendingActions";
...
const EMPTY_DASHBOARD_METRICS: IDashboardMetrics = {
  total: 0,
  approved: 0,
  pending: 0,
  requiresAction: 0,
...
export default class EsgDashboard extends React.Component<
  IEsgDashboardProps,
  IEsgDashboardState
> {
  private readonly service: EsgDashboardService;

  public constructor(props: IEsgDashboardProps) {
    super(props);

    this.service = new EsgDashboardService(props.context);
  ...
  public componentDidMount(): void {
    this.loadDashboard().catch((error: unknown): void => {
      this.handleLoadingError(error);
    });
  }

  public componentDidUpdate(previousProps: IEsgDashboardProps): void {
    if (this.haveDataSourcePropertiesChanged(previousProps)) {
      this.loadDashboard().catch((error: unknown): void => {
        this.handleLoadingError(error);
      });
    }
  ...
  public render(): React.ReactElement<IEsgDashboardProps> {
    const metrics: IDashboardMetrics = this.state.metrics;

    return (
      <section
        className={styles.dashboard}
        aria-label={"Supplier sustainability ESG dashboard"}
      ...
      >
        ...
        {this.state.loading && (
          <div
            ...
            aria-live="polite"
          >
            <Spinner size={SpinnerSize.large} label="Loading ESG dashboard" />
          </div>
        )}

        <div className={styles.kpiRow}>
          <div className={styles.kpiCards}>
            <DashboardKpiCards metrics={metrics} loading={this.state.loading} />
          </div>
        ...
        <div className={styles.analyticsRow}>
          <SubmissionsTrend items={metrics.trend} />

          <SubmissionsByTier items={metrics.byTier} />

          <QualificationBreakdown items={metrics.qualification} />
        </div>

        <div className={styles.analysisRow}>
          <RiskRatingAnalysis items={metrics.risks} />

          <PendingReviewAging items={metrics.aging} />

          <TopPendingActions items={metrics.actions} />

          <AverageScoreByTier items={metrics.averageByTier} />
        </div>

        <div className={styles.bottomRow}>
          <RecentSubmissionsSnapshot
            items={metrics.recent}
            onOpen={this.openRecentSubmission}
          />
  ...
  /**
   * Creates the environment-specific SharePoint list
   * configuration used by EsgDashboardService.
   *
   * Sustainability is mapped to overallPercentage for display.
   * Weighting is used for qualification evaluation.
   */
  ...
  private loadDashboard = async (): Promise<void> => {
    ...
    try {
      const metrics: IDashboardMetrics = await this.service.getDashboard(
  ...
  private openRecentSubmission = (
    item: IDashboardMetrics["recent"][number],
  ): void => {
  ...
  private haveDataSourcePropertiesChanged(
    previousProps: IEsgDashboardProps,
  ): boolean {
    return (
      ...
      previousProps.targetScore !== this.props.targetScore
    );
  }
}
