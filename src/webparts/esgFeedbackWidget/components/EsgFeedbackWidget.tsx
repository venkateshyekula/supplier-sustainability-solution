...
type Recommendation = "Approve" | "Corrective Action" | "Requires Review";
...
interface ITierConfiguration {
  ...
  sustainabilityInternalName: string;
...
const DEFAULT_TIER_CONFIGURATIONS: readonly ITierConfiguration[] = [
  {
    ...
    environmentalCountInternalName: "EnvironmentalQuestionsCount",
    ...
    sustainabilityInternalName: "Tier_x0020_1_x0020_Sustainabilit",
    weightingInternalName: "Tier_x0020_1_x0020_Weighting",
  },
  {
    tier: "Tier 2",
    ...
    sustainabilityInternalName: "Tier_x0020_2_x0020_Sustainabilit",
    weightingInternalName: "Tier_x0020_2_x0020_Weighting",
  },
  {
    tier: "Tier 3",
    listTitle: "Supplier Sustainability Questionnaires Tier 3",
    supplierNameInternalName: "Supplier_x0020_Name",
    emailInternalName: "Email",
    contactNameInternalName: "Name",
    environmentalCountInternalName: "EnvQuestionsCount",
    ...
    sustainabilityInternalName: "Tier_x0020_3_x0020_Sustainabilit",
    weightingInternalName: "Tier_x0020_3_x0020_Weighting",
  },
];
...
const calculateRelativeScore = (
  ...
  return roundToTwoDecimals(Math.min(Math.max(relativeScore, 0), 100));
};
...
const formatDate = (value: string, localeName: string): string => {
  if (!value) {
...
const getCategoryScores = (
  ...
  return [
    {
      name: "Environmental",
      value: parseNumber(item[configuration.environmentalCountInternalName]),
      iconName: "Globe",
      color: "#107c10",
    },
    {
      name: "Social",
      value: parseNumber(item[configuration.socialCountInternalName]),
      iconName: "People",
      color: "#5c2d91",
    },
    {
      name: "Governance",
      value: parseNumber(item[configuration.governanceCountInternalName]),
      iconName: "Bank",
      color: "#0078d4",
    },
  ];
...
export function EsgFeedbackWidget(
  ...
  const tierConfigurations: readonly ITierConfiguration[] = [
    {
      ...
      listTitle:
        props.tier1ListTitle || DEFAULT_TIER_CONFIGURATIONS[0].listTitle,

      sustainabilityInternalName: tier1Fields.overallPercentageInternalName,
    ...
    },
    {
      ...
      listTitle:
        props.tier2ListTitle || DEFAULT_TIER_CONFIGURATIONS[1].listTitle,

      sustainabilityInternalName: tier2Fields.overallPercentageInternalName,
    ...
    },
    {
      ...
      listTitle:
        props.tier3ListTitle || DEFAULT_TIER_CONFIGURATIONS[2].listTitle,

      sustainabilityInternalName: tier3Fields.overallPercentageInternalName,
  ...
  useEffect((): (() => void) => {
    ...
    const fetchLatestItemForTier = async (
      ...
      const selectFields: string[] = [
        ...
        configuration.sustainabilityInternalName,
    ...
    const loadLatestFeedback = async (): Promise<void> => {
      ...
      try {
         ...
         */
        const tierResults: ITierFetchResult[] = await Promise.all(
          tierConfigurations.map(
            (configuration: ITierConfiguration): Promise<ITierFetchResult> => {
         ...
         */
        const availableResults: ITierFetchResult[] = tierResults
          .filter((result: ITierFetchResult): boolean => {
            return Boolean(result.item);
          })
          .sort((first: ITierFetchResult, second: ITierFetchResult): number => {
        ...
        const latestResult: ITierFetchResult | undefined = availableResults[0];
        ...
        const configuration: ITierConfiguration = latestResult.configuration;
        const weightedScore: number = parseNumber(
          item[configuration.weightingInternalName],
        );
        ...
        const itemId: number = typeof item.Id === "number" ? item.Id : 0;
        const created: string =
          typeof item.Created === "string" ? item.Created : "";
        const modified: string =
          typeof item.Modified === "string" ? item.Modified : "";

        const sustainabilityScore: number = parseNumber(
          item[configuration.sustainabilityInternalName],
        );
        setLatestSubmission({
          id: itemId,
          ...
          supplierName:
            getTextValue(item[configuration.supplierNameInternalName]) ||
          ...
          contactName:
            getTextValue(item[configuration.contactNameInternalName]) ||
            item.Author?.Title?.trim() ||
            "",
          ...
          maximumWeightedScore: configuration.maximumWeightedScore,
    ...
    loadLatestFeedback().catch((error: unknown): void => {
      console.error("Unhandled ESG feedback loading error:", error);
  ...
  const openAssessment = (): void => {
    ...
    window.open(latestSubmission.itemUrl, "_blank", "noopener,noreferrer");
  };
  ...
  const openAllAssessments = (): void => {
    ...
    const siteUrl: string = props.context.pageContext.web.absoluteUrl.replace(
      /\/$/,
      "",
    );
  ...
  if (loading) {
    return (
      <div role="status" aria-live="polite" style={{ padding: 16 }}>
        <Spinner size={SpinnerSize.small} label="Loading latest ESG feedback" />
      </div>
    );
  }
      ...
      {!latestSubmission ? (
        ...
        <>
          <div style={supplierRowStyle}>
            ...
            <span style={tierBadgeStyle}>{latestSubmission.tier}</span>
          </div>
          ...
          <div style={scoreLayoutStyle}>
            ...
            >
              <div style={scoreRingInnerStyle}>
                ...
                <span style={scoreCaptionStyle}>Overall ESG Score</span>
                <span style={weightedScoreCaptionStyle}>
                  {roundToTwoDecimals(latestSubmission.sharePointWeightedScore)}{" "}
            ...
            <div>
              {latestSubmission.categoryScores.map(
                (category: ICategoryScore): React.ReactElement => (
...
function DetailBadgeRow(props: IDetailBadgeRowProps): React.ReactElement {
  return (
...
function DetailTextRow(props: IDetailTextRowProps): React.ReactElement {
  return (
    <div style={detailRowStyle}>
      <span style={detailLabelStyle}>{props.label}</span>
